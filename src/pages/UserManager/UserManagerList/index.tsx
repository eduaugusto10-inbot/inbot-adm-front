...
export function UserManagerList() {
  ...
  const [editMode, setEditMode] = useState<Array<boolean>>(
    Array(customers.length).fill(false)
  );
  const [editedValues, setEditedValues] = useState<Array<any>>(
    Array(customers.length).fill({})
  );
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const finalDay =
        qtyDaysFinal < finalDate.day ? qtyDaysFinal : finalDate.day;
      ...
      const initialDate = `${initDate.year}-${String(initDate.month).padStart(
        2,
        "0"
      )}-${String(initDay).padStart(2, "0")}`;
      const lastDate = `${finalDate.year}-${String(finalDate.month).padStart(
        2,
        "0"
      )}-${String(finalDay).padStart(2, "0")}`;
      inbotApi
        .get(
          `/customermanager?initialDate=${initialDate}&finalDate=${lastDate}&limit=1000`
        )
        .then((resp: any) => {
          setCustomers(resp);
  ...
  const updateCustomer = async (index: number) => {
    ...
    for (const elementName of Object.keys(editedValues[0] || {})) {
      if (VALUES.includes(elementName)) {
        ...
        if (customFieldIndex !== -1) {
          updatedCustomFields[customFieldIndex].value =
            editedValues[0][elementName];
        } else {
    ...
    inbotApi.setBotId(Number(botId));
    await inbotApi
      .patch(`/customerManager/customerId/${customerEdited.id}`, requestData)
      .then(() => {
        setEditedValues([]);
        handleSave(index);
  ...
  const createJson = async () => {
    ...
    try {
      ...
      customFields.map((fields: any) => headers.push(fields.id));
      const jsonData: any = [];
      ...
      await Promise.all(
        jsonData.map(async (element: any) => {
      ...
      errorMessageDefault("Erro ao salvar os clientes");
    } finally {
      setLoading(false);
      setIsSaving(false);
    }
  ...
  const handleSort = (outrosDadosFiltrados: any) => {
    let filteredItems = outrosDadosFiltrados.filter((item: any) => {
      // Filtro básico
      const basicFilterValid =
        filterSelected === "" ||
        (item[`${filterSelected}`] &&
          item[`${filterSelected}`]
            .toString()
            .toLowerCase()
            .includes(filterSelectedValue.toLowerCase()));
      ...
      if (advancedFilters.length > 0) {
        advancedFiltersValid = advancedFilters.every((filter) => {
          ...
          if (customFields.some((cf: any) => cf.id === filter.column)) {
            return getCustomFieldValue(item, filter.column)
              .toLowerCase()
    ...
    if (sortOrder === "asc") {
      sortedItems.sort((a, b) => {
        let valorA =
          a[sortType] !== undefined && a[sortType] !== null ? a[sortType] : "Z";
        let valorB =
          b[sortType] !== undefined && b[sortType] !== null ? b[sortType] : "Z";
        if (isCustomFields) {
  ...
  const getCustomFieldValue = (item: any, customFieldId: string) => {
    const customField = item.customFields.find(
      (field: { id: string }) => field.id === customFieldId
    );
  ...
  const deleteCustomer = async (customerId: string) => {
    waitingMessage();
    inbotApi.delete(`/customerManager/customerId/${customerId}`).then(() => {
      ...
      <div className="config-template" style={{ width: "95%" }}>
        <div
          className={`accordion_head ${
            accordionState.new ? "accordion_head_opened" : ""
          }`}
        ...
        >
          ...
          <div className="accordion_chevron">
            <img
              src={chevron}
              alt=""
              style={{ rotate: accordionState.new ? "-90deg" : "90deg" }}
            />
          </div>
        </div>
        {accordionState.new && (
          <div style={{ marginTop: "15px" }}>
            <input
              ...
              id="addUsersType"
              onChange={handleMode}
              checked={accordionTable === true}
              disabled
            />
            <span style={{ padding: "9px" }}>
              Cadastrar usuários individualmente
            </span>
            <input
              type="radio"
              id="addUsersType"
              onChange={handleMode}
              checked={accordionTable === false}
            />
            <span style={{ padding: "9px" }}>
              Upload de planilha de usuários
            </span>
              ...
              <div>
                          ...
                          fileData.slice(1).map((row, rowIndex) => (
                            <tr key={rowIndex}>
                              {row.map((cell, cellIndex) => (
                                <td key={cellIndex} className="border-gray">
                                  <span className="font-size-12">{cell}</span>
                                </td>
                              ))}
                            </tr>
                          ))}
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor:
                        loading || isSaving ? "#c3c3c3" : "#5ed12c",
                    ...
                    disabled={loading || isSaving || fileData.length <= 1}
                  >
                    {loading || isSaving ? (
                      <div className="in_loader"></div>
                    ) : (
                      "Salvar"
                    )}
                  </button>
        ...
        <div
          className={`accordion_head ${
            accordionState.base ? "accordion_head_opened" : ""
          }`}
          style={{ width: "100%", borderRadius: "20px" }}
          ...
          <div>
            <div style={{ margin: "10px 20px", textAlign: "left" }}>
              ...
              <select
                ...
                className="input-values litle-input"
              >
                {[...Array(31).keys()].map((i) => (
              ...
              >
                {loadFinished ? "Buscar" : <div className="in_loader"></div>}
              </button>
            </div>
            <div className="row-align">
              <div
                className="column-align left-align"
              ...
              >
                ...
                <div className="row-align" style={{ alignContent: "center" }}>
                  <span className="color-text-label">
                    Exibir somente usuários ativos:
                  </span>
                  <input
              ...
              >
                ...
                <input
                  ...
                  id=""
                  className="input-values"
                  style={{ border: "none", height: "30px", width: "300px" }}
                />
              </div>
            </div>
            ...
            <div
              className="row-align"
              style={{ justifyContent: "flex-end", margin: "10px 20px" }}
            >
              <button
                className="button-blue"
              ...
              >
                ...
                {advancedFilters.length === 0 ? (
                  <div style={{ margin: "10px 0", color: "#666" }}>
                    Nenhum filtro avançado adicionado. Clique em "Adicionar
                    filtro" para começar.
                  </div>
                ) : (
                  advancedFilters.map((filter, index) => (
                    ...
                    >
                      <select
                        value={filter.column}
                        onChange={(e) =>
                          updateAdvancedFilter(index, "column", e.target.value)
                        }
                        className="input-values"
            ...
            >
              <span className="color-text-label font-size-12">
                {qtyCustomerCounter(qtyCustomer)}
              </span>
              <DownloadTableExcel
              ...
              >
                <thead>
                  <tr className="cells table-2024 border-bottom-zero font-size-12">
                    <th className="cells">
                      ...
                      >
                        <span></span>
                        <span style={{ padding: "0px 15px" }}>ID</span>{" "}
                        <div>
                          <div
                            className="triangle-up"
                            onClick={() =>
                              handleInitSort("phone", "asc", false)
                            }
                          ></div>
                          <div
                            ...
                            style={{ marginTop: "2px" }}
                            onClick={() =>
                              handleInitSort("phone", "desc", false)
                            }
                          ></div>
                        </div>
                      </div>
                    ...
                    <th className="cells">
                      ...
                      >
                        <span></span>
                        <span style={{ padding: "0px 15px" }}>Nome</span>{" "}
                        <div>
                          <div
                            className="triangle-up"
                            onClick={() => handleInitSort("name", "asc", false)}
                          ></div>
                          <div
                            className="triangle-down"
                            style={{ marginTop: "2px" }}
                            onClick={() =>
                              handleInitSort("name", "desc", false)
                            }
                          ></div>
                        </div>
                      </div>
                    ...
                    <th className="cells">
                      ...
                      >
                        <span></span>
                        <span style={{ padding: "0px 15px" }}>E-mail</span>{" "}
                        <div>
                          <div
                            className="triangle-up"
                            onClick={() => handleInitSort("name", "asc", false)}
                        ...
                        >
                          ...
                          <div>
                            <div
                              className="triangle-up"
                              onClick={() =>
                                handleInitSort(fields.id, "asc", true)
                              }
                            ></div>
                            <div
                              className="triangle-down"
                              style={{ marginTop: "2px" }}
                              onClick={() =>
                                handleInitSort(fields.id, "desc", true)
                              }
                            ></div>
                          </div>
                        </div>
                      ...
                      >
                        ...
                        <div>
                          <div
                            className="triangle-up"
                            onClick={() =>
                              handleInitSort("createdAt", "asc", false)
                            }
                          ></div>
                          <div
                            className="triangle-down"
                            style={{ marginTop: "2px" }}
                            onClick={() =>
                              handleInitSort("createdAt", "desc", false)
                            }
                          ></div>
                        </div>
                      </div>
                ...
                <tbody>
                  {handleSort(customers).map((customer: any, index: number) => (
                    ...
                    >
                      <td className="border-gray">
                        <span className="font-size-12">{customer.id}</span>
                      </td>
                      <td className="border-gray">
                      ...
                      <td className="cells border-gray">
                        <span className="font-size-12">
                          {editMode[index] ? (
                            <input
                              type="text"
                              ...
                              value={
                                editMode[index] &&
                                hasShowNameAndValue(editedValues, "name", false)
                                  ? editedValues[0]?.name
                                  : customer.name
                              }
                      ...
                      {customFields.map((fields: any, key: any) => (
                        <td className="border-gray" key={key}>
                          <span className="font-size-12">
                            {editMode[index] ? (
                              <input
                                type="text"
                                ...
                                value={
                                  editMode[index] &&
                                  editedValues[0] &&
                                  editedValues[0][fields.id] !== undefined
                                    ? editedValues[0][fields.id]
                                    : showNameAndValue(customer, fields, true)
                                }
                                onChange={(e) => handleChange(e, 0, fields.id)}
                              />
                            ) : (
                              showNameAndValue(customer, fields, true)
                            )}
                      ...
                      <td className="border-gray">
                        <span style={{ fontSize: "12px" }}>
                          {adjustTimeWithout3Hour(customer.createdAt)}
                        </span>
                      </td>
                      <td className="cells border-gray">
                        <span className="font-size-12">
                          {editMode[index] ? (
                            <select
                              ...
                              style={{ width: "80px" }}
                              value={
                                editMode[index] && editedValues[0]?.activated
                                  ? editedValues[0]?.activated
                                  : customer.activated
                              }
                              onChange={(e) => handleChange(e, 0, "activated")}
                            >
                            ...
                            <div id="statusCells">
                              <span
                                style={{
                                  fontWeight: "bolder",
                                  color: customer.activated ? "green" : "red",
                                }}
                              >
                                {customer.activated ? "Ativo" : "Inativo"}
                              </span>
                            </div>
                          )}
                        </span>
                        ...
                        {editMode[index] ? (
                          ...
                          >
                            <button
                              ...
                              onClick={() => openUpdateModal("Alterar", index)}
                            >
                              Salvar
                            </button>
                            <button
                          ...
                          <div>
                            {
                              <span onClick={() => toggleEditMode(index)}>
                                <img
                                  src={pencil}
                                  width={15}
                                  height={15}
                            ...
                            <span
                              ...
                              onClick={() =>
                                openDeletarModal("Deletar", customer.id)
                              }
                            >
                              <img
                                src={trash}
