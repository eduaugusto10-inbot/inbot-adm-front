...
export function Accordion() {
  ...
  const handleSubmitManualListData = async (campaignId: string) => {
    for (let i = 0; i < listVariables.length; i++) {
      ...
      api.post("/whats-customer", params).catch((error) => console.log(error));
    }
  };
  ...
  const handleDispatchNumberChange = (
    ...
    if (selectedNumber !== "") {
      ...
      if (errorMessage && errorMessage.includes("número de disparo")) {
        setErrorMessage("");
      }

      // Make API call to get trigger names for the selected number
      api
        .get(`/whatsapp/trigger-bot/${botId}?number=${selectedNumber}`)
        .then((resp) => setTriggerNames(resp.data))
        .catch((error) => console.log(error));
    }
  };
  ...
  return (
    ...
    >
            ...
            <div className="accordeon-new">
              ...
              <div style={{ width: "100%", textAlign: "right" }}>
                <button
                  ...
                  onClick={() => toggleAccordion("selectNumber")}
                >
                  Próximo
                </button>
              </div>
      ...
      <div>
        <div
          className={`accordion_head ${
            accordionState.selectNumber ? "accordion_head_opened" : ""
          }`}
          style={{ borderRadius: "20px" }}
          onClick={() => toggleAccordion("selectNumber")}
        >
          ...
          <div className="accordion_chevron">
            <img
              ...
              alt=""
              style={{
                rotate: accordionState.selectNumber ? "-90deg" : "90deg",
              }}
            />
          </div>
        </div>
      ...
      >
        <div
          ...
          onClick={() => toggleAccordion("recebidores")}
        >
          4. Cadastro dos Contatos da Campanha
          <div className="accordion_chevron">
            <img
              src={chevron}
              alt=""
