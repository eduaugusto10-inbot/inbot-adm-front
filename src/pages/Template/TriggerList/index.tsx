            ...
            <button onClick={()=>setChangeDateFilter(previous => !previous)} className="button-blue">Buscar</button>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", width:"100%", margin: "10px 0px" }}>
                        <div className="row-align" style={{marginBottom:"30px", alignItems:"center"}}>
                            ...
                            <div className={filters.status.aguardando ? "border_gradient" : "border_gradient-gray"} style={{marginRight:"15px", cursor:"pointer", marginLeft:"20px"}} onClick={()=>""}><div className={filters.status.aguardando ? "number_button_gradient" : "number_button_gradient-gray"}  onClick={() => handleStatusChange('aguardando')}>Aguardando</div></div>
                            <div className={filters.status.enviado ? "border_gradient" : "border_gradient-gray"} style={{marginRight:"15px", cursor:"pointer"}} onClick={()=>""}><div className={filters.status.enviado ? "number_button_gradient" : "number_button_gradient-gray"}  onClick={() => handleStatusChange('enviado')}>Enviado</div></div>
                            <div className={filters.status.erro ? "border_gradient" : "border_gradient-gray"} style={{marginRight:"15px", cursor:"pointer"}} onClick={()=>""}><div className={filters.status.erro ? "number_button_gradient" : "number_button_gradient-gray"}  onClick={() => handleStatusChange('erro')}>Erro</div></div>
                            <div className={filters.status.cancelado ? "border_gradient" : "border_gradient-gray"} style={{marginRight:"15px", cursor:"pointer"}} onClick={()=>""}><div className={filters.status.cancelado ? "number_button_gradient" : "number_button_gradient-gray"}  onClick={() => handleStatusChange('cancelado')}>Cancelado</div></div>
                        </div>
                    </div>
                    </div>
                </div>
